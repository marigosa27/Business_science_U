...
bike_orderlines_tbl %>% 
    ...
    #mutate(Aluminum = replace_na(Aluminum,0),
    #       Carbon = replace_na(Carbon,0)) %>% 
    group_by(Aluminum, Carbon) %>% 
           mutate(total_sales = sum(Aluminum,Carbon)) %>% 
    ungroup() %>% 
